...
import ModalEmbarcacaoCadastro from "@/src/app/components/portal/modalEmbarcacaoCadastro/index"
...
const Page = () => {
  ...
  const fetchData = async () => {
    ...
    if (token) {
      if (isTokenExpired(token)) {
        ...
        try {
          const response = await axios.get("/api/portal/embarcacao/readVessel", {
            headers: {
              Authorization: `Bearer ${token}`,
            }
          });
          const embarcacoesLimpas = response.data.map((embarcacao:any) => ({
            ...embarcacao,
  ...
  const deleteEmbarcacao = async (inscricao: any) => {
    ...
    try {
      ...
      setEmbarcacoes((prevEmbarcacoes) =>
        prevEmbarcacoes.filter((embarcacao) => embarcacao.Inscricao !== inscricao)
      );
      pesquisaEmbarcacao();
    } catch (error) {
  ...
  const [embarcacaoToDelete, setEmbarcacaoToDelete] = useState<string | null>(null);
  ...
  const renderIcons = (rowData: any) => {
    return (
      <div className={styles.iconCenter}>
        <i className={`pi pi-pen-to-square ${styles['icon-bold']}`} onClick={() => handleEdit(rowData)}></i>
        <i className={`pi pi-trash ${styles['icon-bold']}`} onClick={() => {
  ...
  return (
    <>
      {modal && <ModalEmbarcacaoCadastro onClose={onClose} onSuccess={onSuccess} embarcacao={selectedEmbarcacoes} />}
      <div className={styles.container}>
        ...
        <div className={styles.containerPesquisa}>
          <InputSearch type='inputIcon' icon='pi pi-search' placeholder='Pesquise uma Embarcação' value={searchTerm} onChange={handleSearch} />
          <Button tipoBotao='normal' title='Novo' icon='pi pi-plus-circle' tamanho='11em' onClick={openModal} />
        </div>
        ...
        {loading ? (
          ...
          <DataTable value={embarcacoesFiltrada} showGridlines tableStyle={{ minWidth: '10em' }} paginator rows={10}
            className={`${styles.tabela} ${styles['rounded-datatable']}`}
            emptyMessage="Nenhuma embarcação disponível" // Mensagem quando não há dados
          >
            <Column field="Embarcacao" header="Embarcação" headerStyle={{ color: 'black', backgroundColor: 'white', maxWidth: '27em' }}></Column>
            <Column field="empresa.Empresa" header="Empresa" headerStyle={{ color: 'black', backgroundColor: 'white', width: 'auto' }}></Column>
            <Column field="Inscricao" header="Inscrição" headerStyle={{ color: 'black', backgroundColor: 'white', width: 'auto' }}></Column>
            <Column header="Ações" body={renderIcons} headerStyle={{ textAlign: 'center', alignContent: 'center', backgroundColor: 'white', width: '4em', color: 'black' }} bodyStyle={{ justifyContent: 'space-between' }}></Column>
          </DataTable>
        )}
      </div>
      <ConfirmDialog
