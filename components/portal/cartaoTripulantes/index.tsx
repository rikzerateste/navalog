...
const Page = () => {
  ...
  const handleInfos = async (controle: number) => {
    ...
    try {
      ...
      const ctsResponse = await axios.get(`/api/portal/cts/getCtsForPdf?controle=${controle}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      console.error("Erro ao buscar as informações do pedido de despacho", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  async function btnReportGenerate(reportData: any) {
    ...
    jsreport.headers['Authorization'] = "Basic " + btoa("davisantana.ti@gmail.com:bc2024")
    ...
    try {
      ...
      const report = await jsreport.render({
        ...
        template: { name: 'cts-template' }, data: jsonData, options: { "office": { "preview": true } }
      });

      report.openInWindow({ title: "Cartao de Tripulacao de Seguranca" })
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : 'Report unknown error'
  ...
  const renderIcons = (rowData: any) => {
    return (
      <div className={styles.iconCenter}>
        <i className={`pi pi-file-export ${styles['icon-bold']}`} onClick={() => handleInfos(rowData.CTS_controle)}></i>
        <i className={`pi pi-pen-to-square ${styles['icon-bold']}`} onClick={() => handleEdit(rowData)}></i>
        <i className={`pi pi-trash ${styles['icon-bold']}`} onClick={() => deleteCTS(rowData.CTS_controle)}></i>
      </div>
    );
  };
  ...
  return (
    <>
      {modal && <ModalCTSCadastro onClose={onClose} onSuccess={onSuccess} cts={selectedCTS} />}
