...
const Page = () => {
  ...
  const handleInfos = async (pedido: number) => {
    ...
    try {
      const pedidoResponse = await axios.get(`/api/portal/pedidoDespacho/getDispatchRequestForPdf?pedido=${pedido}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  async function btnReportGenerate(reportData: any) {
    ...
    try {
      ...
      const report = await jsreport.render({
        template: { name: 'pedidodedespacho-template' }, data: jsonData, options: { "office": { "preview": true } }
      });

      report.openInWindow({ title: "Pedido de Despacho por Periodo - Navegacao Interior" })
    } catch (error) {
  ...
  return (
    <>
      {isModalOpen && (
        <ModalDespachoCadastro onClose={onClose} onSuccess={onSuccess} despacho={selectedDespacho} />
      )}
